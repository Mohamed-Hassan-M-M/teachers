                        ...
                        <div id="html-validations" class="card card-tabs">
                            <div class="card-content">
                                <div class="card-title">
                                    <div class="row">
                                        <div class="col s12 m6 l10">
                                            <h4 class="card-title">@lang('general.update') @lang('general.classes')</h4>
                                        </div>
                                ...
                                <div id="html-view-validations">
                                    <form class="formValidate0" id="formValidate0" action="{{route('admin.classes.update', $subject->id)}}" method="post">
                                        @method('PUT')
                                        @csrf
                                        <div class="row">
                                            <div class="col s12">
                                                ...
                                                <select id="sector_id" name="sector_id" required>
                                                    <option value="" disabled>@lang('general.choose') @lang('general.sector')</option>
                                                    @foreach($sectors as $sector)
                                                        <option value="{{$sector->id}}" @if($class->sector->id == $sector->id) selected @endif>{{$sector->type}} -> {{$sector->name}}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            @foreach(['ar', 'en'] as $local)
                                                <div class="input-field col s6">
                                                    ...
                                                    <input @error('name_'.$local)class="invalid" @enderror required value="{{old('name_'.$local, $class['name_'.$local])}}" id="name_{{$local}}"
                                                           name="name_{{$local}}"
                                                           type="text">
                                                    @error('name_'.$local)
                                                        <span class="red-text" role="alert">
